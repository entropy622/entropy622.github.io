...
export default defineConfig({
  // [Basic]
  site: 'https://entropy622.github.io',
  ...
  integrations: [
    ...
    sitemap({
      // Keep canonical pages in the sitemap. Hidden posts and compatibility
      // redirects remain reachable, but should not be advertised as content.
      filter: (page) => {
        const pathname = decodeURIComponent(new URL(page).pathname)
        return (
          !pathname.endsWith('/blog/paper-changan-internship') &&
